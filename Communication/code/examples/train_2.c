...
int main() {
	int liste_parcourt[50] = {44,7,11,63,27,28,13,49,12,24,60,5,6,44};/*à remplir avec le parcourt */
	...
	printf("[TRAIN 2]Bien connecté au serveur g2r\n");
	...
	while(1){ //le train tourne en permanance sur son circuit
		...
		for(i = 0; i <length_parcourt; i++){
			...
			printf("[TRAIN 2]mon service demandé est : %d\n",mon_service);
			...
			if (is_ressource_fun(mon_service)){
				...
				printf("[TRAIN 2]je demande l'accession à l'API\n");
				sleep(4);
				...
				printf("[TRAIN 2]réation et envoi de la trame XWAY à à l'API pour ce service non critique \n");
