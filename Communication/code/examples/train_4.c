...
#define TRAIN_ID 4 // Numéro du train
#define TRAME_ID 4
...
int main() {
	int liste_parcourt[50] = {47,10,29,28,27,53,29,43,33,49,13,28,27,53,11,27,28,29,10,47};/*à remplir avec le parcourt */
	...
	printf("[TRAIN 4]Bien connecté au serveur g2r\n");
	...
	while(1){ //le train tourne en permanance sur son circuit
		...
		for(i = 0; i <length_parcourt; i++){
			...
			printf("[TRAIN 4]mon service demandé est : %d\n",mon_service);
			...
			if (is_ressource_fun(mon_service)){
				...
				printf("[TRAIN 4]je demande l'accession à l'API\n");
				sleep(7);
				...
				printf("[TRAIN 4]réation et envoi de la trame XWAY à à l'API pour ce service non critique \n");
